  ...
  VideoBloc() : super(InitState()) {
    ...
    on<InitEvent>((event, emit) async {
      ...
      try {
        ...
        if (event.videoPageModel.videoId == BigInt.parse('0')) {
          var lastActionIdDataSet = await videoRepository
              .lastActionId(['*'], event.videoPageModel.userId);
          event.videoPageModel.videoId =
              lastActionIdDataSet.firstValueWithType<BigInt>('data', 'id',
        ...
        var tblVidVideoMainDataSet = await appRepositories.tblVidVideoMain(
            ...
            id: event.videoPageModel.videoId,
            getNoSqlData: 1);
        var tblVidVideoAchvMapDataSet =
            await appRepositories.tblVidVideoAchvMap(
                'Video/GetObject', ['id', 'achv_id', 'video_id'],
                video_id: event.videoPageModel.videoId);
        var tblUserMainDataSet = await appRepositories.tblUserMain(
            'Video/GetObject', ['id', 'country_id'],
            id: event.videoPageModel.userId);
        var tblLocL1Country = await appRepositories
            .tblLocL1Country('Video/GetObject', ['id', 'lang', 'countrycode']);

        if (tblVidVideoMainDataSet.firstValue(
        ...
        if (event.videoPageModel.videoData == null &&
            event.videoPageModel.videoId != null &&
            event.videoPageModel.videoId != BigInt.parse('0')) {
          BigInt vId = event.videoPageModel.videoId ?? BigInt.parse('0');
          event.videoPageModel.videoData =
              await videoRepository.downloadVideo(videoId: vId);
        } else {
          event.videoPageModel.videoData = null;
        }

        event.videoPageModel.videoObjectId ??=
            tblVidVideoMainDataSet.firstValue('data', 'video_path');
        ...
        VideoControllerProvider.createController(
          ...
          if (videoPlayerController != null) {
            ...
            event.videoPageModel.videoUploadStatusText =
        ...
        var isPublic = tblVidVideoMainDataSet.firstValue('data', 'is_public',
            insteadOfNull: 1);
        ...
        event.videoPageModel.videoTitle = tblVidVideoMainDataSet
            .firstValue('data', 'title', insteadOfNull: '');
        event.videoPageModel.videoDescriptions = tblVidVideoMainDataSet
            .firstValue('data', 'description', insteadOfNull: '');

        event.videoPageModel.selectedCountry = tblVidVideoMainDataSet
            .firstValue('data', 'country', insteadOfNull: 0);
        if (event.videoPageModel.selectedCountry == 0 ||
            event.videoPageModel.selectedCountry == null) {
          event.videoPageModel.selectedCountry = tblUserMainDataSet
              .firstValue('data', 'country_id', insteadOfNull: 0);
          if (event.videoPageModel.selectedCountry == 0 ||
              event.videoPageModel.selectedCountry == null) {
            event.videoPageModel.selectedCountry = tblLocL1Country.firstValue(
                'data', 'id',
            ...
            if (event.videoPageModel.selectedCountry == 0 ||
                event.videoPageModel.selectedCountry == null) {
              event.videoPageModel.selectedCountry =
                  tblLocL1Country.firstValue('data', 'id');
            }
          }
        }
        if (event.videoPageModel.selectedCountry != null &&
            event.videoPageModel.selectedCountry != 0) {
        ...
        var tblUtilAcademicYearDataSet = await appRepositories
            .tblUtilAcademicYear(
                'Video/GetObject', ['id', 'is_default', 'acad_year']);
        event.videoPageModel.selectedAcademicYear = tblVidVideoMainDataSet
            .firstValue('data', 'academic_year', insteadOfNull: 0);
        if (event.videoPageModel.selectedAcademicYear == 0 ||
            event.videoPageModel.selectedAcademicYear == null) {
          event.videoPageModel.selectedAcademicYear =
              tblUtilAcademicYearDataSet.firstValue('data', 'id',
                  filterColumn: 'is_default',
                  filterValue: true,
                  insteadOfNull: 0);
        }
        event.videoPageModel.academicYears = tblUtilAcademicYearDataSet
        ...
        var tblClassGradeDataSet = await appRepositories.tblClassGrade(
            'Video/GetObject', ['id', 'grade_name', 'country_id'],
            country_id: event.videoPageModel.selectedCountry);
        event.videoPageModel.selectedGrade = tblVidVideoMainDataSet
            .firstValue('data', 'grade_id', insteadOfNull: 0);
        if (event.videoPageModel.selectedGrade == 0 ||
            event.videoPageModel.selectedGrade == null) {
          event.videoPageModel.selectedGrade =
              tblClassGradeDataSet.firstValue('data', 'id');
        }
        event.videoPageModel.grades = tblClassGradeDataSet
            .toKeyValuePairsWithTypes<int, String>('data', 'id',
                valueColumn: 'grade_name');

        var tblTheaBranMapDataSet = await appRepositories.tblTheaBranMap(
            'Video/GetObject', ['id', 'branch_id', 'user_id'],
            user_id: event.videoPageModel.userId);
        event.videoPageModel.selectedBranch = tblVidVideoMainDataSet
            .firstValue('data', 'branch_id', insteadOfNull: 0);
        if (event.videoPageModel.selectedBranch == 0 ||
            event.videoPageModel.selectedBranch == null) {
          event.videoPageModel.selectedBranch = tblTheaBranMapDataSet
              .firstValue('data', 'branch_id', insteadOfNull: 0);
        }
        var tblLearnBranch = await appRepositories.tblLearnBranch(
            'Video/GetObject', ['id', 'branch_name', 'country_id'],
            country_id: event.videoPageModel.selectedCountry);
        event.videoPageModel.branches =
            tblLearnBranch.toKeyValuePairsWithTypes<int, String>('data', 'id',
                valueColumn: 'branch_name');

        event.videoPageModel.selectedSubDomain = tblVidVideoMainDataSet
            .firstValue('data', 'subdom_id', insteadOfNull: 0);
        if (event.videoPageModel.selectedSubDomain == 0 ||
            event.videoPageModel.selectedSubDomain == null) {
        } else {
          var tblLearnSubdomainDataSet = await appRepositories
              .tblLearnSubdomain('Video/GetObject', ['id', 'domain_id']);
          event.videoPageModel.selectedDomain =
              tblLearnSubdomainDataSet.firstValue('data', 'domain_id',
                  filterColumn: 'id',
                  filterValue: event.videoPageModel.selectedSubDomain);
        }

        if (event.videoPageModel.selectedBranch != 0 &&
            event.videoPageModel.selectedBranch != null) {
          var tblLearnDomainDataSet = await appRepositories.tblLearnDomain(
              'Video/GetObject', ['id', 'domain_name', 'branch_id'],
              branch_id: event.videoPageModel.selectedBranch);
          event.videoPageModel.domains = tblLearnDomainDataSet
              .toKeyValuePairsWithTypes<int, String>('data', 'id',
                  valueColumn: 'domain_name');
        }

        if (event.videoPageModel.selectedDomain != 0 &&
            event.videoPageModel.selectedDomain != null) {
          var tblLearnSubdomainDataSet =
              await appRepositories.tblLearnSubdomain(
                  'Video/GetObject', ['id', 'subdom_name', 'domain_id'],
                  domain_id: event.videoPageModel.selectedDomain);
          event.videoPageModel.subDomains = tblLearnSubdomainDataSet
              .toKeyValuePairsWithTypes<int, String>('data', 'id',
        ...
        if (event.videoPageModel.selectedSubDomain != 0 &&
            event.videoPageModel.selectedSubDomain != null) {
          var tblLearnAchivementDataSet = await videoRepository
              .getAchievementsFromSubDomainId(
                  ['id', 'achivement_text', 'subdom_id', 'country_id'],
                  subdom_id: event.videoPageModel.selectedSubDomain,
                  country_id: event.videoPageModel.selectedCountry);
          event.videoPageModel.achievements = tblLearnAchivementDataSet
              .toKeyValuePairsWithTypes<int, String>('data', 'id',
                  valueColumn: 'achivement_text');
        }
        event.videoPageModel.achievementsBulk = await videoRepository
            .getAchievementsFromSubDomainId(
                ['id', 'achivement_text', 'subdom_id', 'country_id']);

        var learnId = tblVidVideoMainDataSet.firstValue('data', 'subdom_id',
            insteadOfNull: 0);
        event.videoPageModel.selectedLearn = learnId;

        if (event.videoPageModel.selectedLearn == 0 ||
            event.videoPageModel.selectedLearn == null) {
        } else {
          //Get Achievements
          var tblLearnMainDataSet = await appRepositories.tblLearnMain(
              'Video/GetObject',
              ['id', 'name', 'branch_id', 'parent_id', 'country_id', 'type'],
              parent_id: event.videoPageModel.selectedLearn,
              country_id: event.videoPageModel.selectedCountry,
              type: 'ct_achv');
          var achievements = tblLearnMainDataSet
              .toKeyValuePairsWithTypes<int, String>('data', 'id',
                  valueColumn: 'name');
          event.videoPageModel.achievements = achievements;

          var firtsAchievementId = achievements.entries.first.key;
          var branchId = tblLearnMainDataSet.firstValue('data', 'branch_id',
              insteadOfNull: event.videoPageModel.selectedBranch);
        ...
        if (setAgainRootIdAsZero) {
          ...
          event.videoPageModel.isVideoContainerExpanded = false;
          event.videoPageModel.videoUploadStatusText = AppLocalization.instance
              .translate('lib.bloc.bloc.video.videoBloc', 'InitEvent',
                  'videoNotExist');
        }
    ...
    on<SavePmEvent>((event, emit) async {
      ...
      try {
        var tblUserSubuserDataSet = await appRepositories.tblUserSubuser(
            ...
            sub_user_id: event.videoPageModel.userId);
        ...
        var tblVidVideoMainDataSet = await appRepositories.tblVidVideoMain(
            ...
            id: event.videoPageModel.videoId, getNoSqlData: 0);

        BigInt? videoId = tblVidVideoMainDataSet.firstValueWithType<BigInt>(
            'data', 'id',
            insteadOfNull: 0); //  if Question not exist to insert new one.
        event.videoPageModel.videoId = videoId;

        BigInt? createdBy = tblVidVideoMainDataSet.firstValueWithType<BigInt>(
            'data', 'created_by',
            insteadOfNull: BigInt.parse('0'));
        BigInt? updatedBy = tblVidVideoMainDataSet.firstValueWithType<BigInt>(
            'data', 'updated_by',
            insteadOfNull: BigInt.parse('0'));
        DateTime? createdOn = tblVidVideoMainDataSet
            .firstValueWithType<DateTime>('data', 'created_on',
                insteadOfNull: DateTime.now());
        double? aggRating = tblVidVideoMainDataSet.firstValueWithType<double?>(
            'data', 'aggRating',
            insteadOfNull: double.parse('0'));
        ...
        setObject.userId =
            isCorporateUser ? mainUserId : event.videoPageModel.userId;
        ...
        setObject.tblVidVideoMain = TblVidVideoMain(
          ...
          videoPath: event.videoPageModel.fileNameOfUploaded,
          //event.videoPageModel.videoObjectId ?? '',
          ...
          createdBy: createdBy == BigInt.parse('0')
              ...
              : createdBy,
          createdOn: event.videoPageModel.videoId == BigInt.parse('0')
              ? DateTime.now()
              : createdOn,
