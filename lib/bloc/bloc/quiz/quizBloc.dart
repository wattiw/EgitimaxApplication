  ...
  QuizBloc() : super(InitState()) {
    ...
    on<InitEvent>((event, emit) async {
      ...
      try {
        ...
        var tblQuizMainDataSet = await appRepositories
            .tblQuizMain('Quiz/GetObject',['id','country', 'academic_year', 'user_id', 'grade_id', 'title', 'description', 'duration', 'header_text', 'footer_text', 'is_public', 'is_active', 'agg_rating', 'created_by', 'created_on', 'updated_by', 'updated_on'], id: event.quizPageModel.quizId);
        var tblQuizSectionDataSet = await appRepositories.tblQuizSection(
            'Quiz/GetObject',['id','quiz_id','branch_id','order_no','section_desc','is_active'],
            quiz_id: event.quizPageModel.quizId);
        var tblUserMainDataSet = await appRepositories
            .tblUserMain('Quiz/GetObject',['id','country_id'], id: event.quizPageModel.userId);
        var tblLocL1Country = await appRepositories.tblLocL1Country(
          'Quiz/GetObject',['id','countrycode','lang'],
        );
        ...
        if (countryId != null && countryId != 0) {
          ...
          for (var country in countries.entries) {
            ...
            var tblLocL1CountryTrans = await appRepositories
                .tblLocL1CountryTrans('Quiz/GetObject',['id','country_name','lang_id'], lang_id: countryLangId);
        ...
        BigInt? userId =
            tblQuizMainDataSet.firstValue('data', 'user_id', insteadOfNull: BigInt.parse('0'));
        ...
        for (var section in tblQuizSectionDataSet.selectDataTable('data').toList()) {
          ...
          try {
            ...
            try {
              var tblQuizSectQuestMapDataSet = await appRepositories
                  .tblQuizSectQuestMap('Quiz/GetObject',['id','section_id','question_id','order_no','is_active'], section_id: qs.id);
              for (var map in tblQuizSectQuestMapDataSet.selectDataTable('data').toList())
              {
        ...
        var tblTheaBranMapDataSet = await appRepositories.tblTheaBranMap('Quiz/GetObject',['id','branch_id','user_id'],user_id: event.quizPageModel.userId);
        var userBranchId=tblTheaBranMapDataSet.firstValue('data','branch_id',insteadOfNull: 0);

        if (event.quizPageModel.quizMain!.quizSections == null || event.quizPageModel.quizMain!.quizSections!.isEmpty) // Default : Main Section Added if not any section
        {
          QuizSection qs = QuizSection();
        ...
        var tblLearnBranch = await appRepositories.tblLearnBranch('Quiz/GetObject',['id','branch_name','country_id'], country_id: event.quizPageModel.quizMain?.country);
        event.quizPageModel.branches =tblLearnBranch.toKeyValuePairsWithTypes<int, String>('data', 'id',valueColumn: 'branch_name');
